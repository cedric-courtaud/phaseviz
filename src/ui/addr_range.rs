...
impl<'a> Panel<'a> for InstAddrPanel<'a> {
    ...
    fn render_header<B, I>(&'a self, f: &mut Frame<B>, _items: I, ctx: &Self::Context) where B: Backend, I: AsRef<[&'a ProfileItem]> {
        let p = Paragraph::new(Span::from(format!("{:^30}", "Inst addr range"))).block(ctx.pbox.header.block.clone());
    ...
    fn render_body<B, I>(&'a self, f: &mut Frame<B>, items: I, ctx: &Self::Context) where B: Backend, I: AsRef<[&'a ProfileItem]>{
        ...
        let p = Paragraph::new(Text::from(lines)).block(ctx.pbox.body.block.clone().borders(Borders::NONE));
